...
def _section_skill(project: Project) -> str:
    ...
    if project.render_backend == "remotion":
        head += (
            ...
            "(`engine/clipwright/render_*.py`), captions, audio, or "
...
def _section_scope(project_dir: Path) -> str:
    ...
    return (
        ...
        "\n"
        "- The Clipwright installation itself — `engine/clipwright/…`, "
        ...
        "  root.\n"
        "\n"
        "**FORBIDDEN file kinds — do not create or edit these even inside "
        "the project tree:**\n"
        "\n"
        "- **Source code**: no `.tsx`, `.ts`, `.jsx`, `.js`, `.py`, `.rs`, "
        "  `.go`, `.rb`, `.java`, `.swift`, `.kt`, `.c`, `.cpp`, `.h`, "
        "  `.mjs`, `.cjs` — Clipwright is a no-code editor for the user. "
        "  If a beat seems to need new logic (e.g. \"a cyberpunk-themed "
        "  animated outro\"), DO NOT write a Remotion component. Either "
        "  parametrize one of the existing built-in scenes (e.g. by "
        "  picking a `chapter` value the renderer already handles) or "
        "  report a 'Known limitation' to the user — like the title-card "
        "  segment IS the animated outro (its `camera.<seg>.json` gives "
        "  you the animation knobs).\n"
        "- **Build / config**: no `package.json`, `tsconfig*.json`, "
        "  `Cargo.toml`, `pyproject.toml`, `vite.config.*`, "
        "  `tauri.conf.json`, `Dockerfile`, `Makefile`, etc.\n"
        "- **CI / git**: no `.github/`, `.gitignore`, `.gitattributes`.\n"
        "\n"
        "**ALLOWED file kinds** are data only: `.json` (project, "
        "videos/<id>, voiceover scripts, camera, annotations, "
        "captions/style), `.md` (planning notes in `notes/`), `.mp3` / "
        "`.wav` / `.mp4` / `.webp` / `.png` / `.jpg` (media in "
        "`sources/`, `voiceover/audio/`, `captions/`, `out/`), and the "
        "cache sidecars (`.cache.json`) the pipeline manages on its own.\n"
        "\n"
        "If a task seems to require editing outside the project, OR a "
        "file kind in the FORBIDDEN list (e.g. \"fix the clipwright CLI "
        "to do X\" or \"write a custom Remotion outro component\"), STOP "
        "and tell the user that the rail is project-scoped and "
        "data-only; they need to make code changes from a code editor "
        "against the clipwright repo, not from this chat.\n"
        "\n"
...
def _section_constraints(*, focus: Segment | None) -> str:
    lines = [
        ...
        "`engine/clipwright/schema/v2/segment.py` (or `clipwright/schema/v2/segment.py` if installed) "

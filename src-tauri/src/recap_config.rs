...
//! `engine/clipwright/recap_config.py` owns the schema + I/O; here we
...
fn default_outro_duration() -> f64 {
    ...
    // `engine/clipwright/recap_config.py`. If either side changes the
...
fn default_target_duration() -> u64 {
    ...
    // `engine/clipwright/recap_config.py`.
    90
}
...
pub struct RecapConfig {
    ...
    /// `RecapConfig.persona` in `engine/clipwright/recap_config.py`.
    #[serde(default)]
    pub persona: String,
